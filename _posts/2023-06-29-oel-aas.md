...
## _Setting the Table_
### Introduction
IBM Z and Cloud Modernization Stack is a relatively new and exciting offering from IBM that provides multiple components that clients can use as a part of their z/OS modernization journey. The stack has components to assist with modernization existing and new z/OS application development scenarios. Here's a brief description of the stack's components:
...
- Get the API server for OpenShift cluster
  - <details open>
    <summary>From OpenShift UI <i><strong> (click to collapse) </strong></i> </summary>
...
- Decide on login method to use:
  - <details open>
    <summary>OpenShift Auth Token <i><strong> (click to collapse) </strong></i> </summary>
  ...
  - OR - [1Password Integration](https://github.com/IBM/zmodstack-solutions/blob/blog-seaa-post/docs/howto/integrate-with-1password-cli.md)
...
- **Step 0:** Get the Code and Preform Pre-req Steps
...
- **Step 1:** Create an inventory file
  - Copy [sample inventory file](https://github.com/IBM/zmodstack-solutions/blob/main/ibm/seaa/ansible/playbooks/inventory/sample-inventory.yaml) to your default inventory location based on `SEAA_CONFIG_PATH_TO_SE_ANSIBLE_ARTIFACTS` configuration from **Step 0**
  ...
  <details open>
    <summary>See video of copying sample inventory and updating 'ocphosts'<i><strong> (click to collapse) </strong></i> </summary>
    <div class="video-container">
      <video controls autoplay loop muted style="width: 100%;">
  ...
  <details open>
    <summary>See video of updating 'zosendpoints'<i><strong> (click to collapse) </strong></i> </summary>
    <div class="video-container">
      <video controls autoplay loop muted style="width: 100%;">
...
- **Step 2** Run script to deploy IBM Open Enterprise Languages
  ...
  <!-- - <details open> -->
    <!-- <summary> <i>./run-deploy-oel-dev-env.sh --tags=oel-dev</i> <i><strong> (click to collapse) </strong></i> </summary> -->
...
- **On OpenShift**
  - OpenShift Project
    ...
    - IBM z/OS Package Manager Operator
      - Customer resources for z/OS Products and Validations deployed
